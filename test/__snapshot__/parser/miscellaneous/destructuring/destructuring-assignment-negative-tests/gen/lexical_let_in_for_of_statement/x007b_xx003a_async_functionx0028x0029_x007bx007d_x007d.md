...
- Path: kataw8/test\__snapshot__\parser\miscellaneous\destructuring\destructuring-assignment-negative-tests\gen\lexical_let_in_for_of_statement
> :: test: lexical let in for of statement
> :: case: { x: async function() {} }
## Input

`````js
'use strict'; let x, y, z; for (x of { x: async function() {} } = {});
`````
## Output
...
```javascript
{
    ...
    "isModule": false,
    "source": "'use strict'; let x, y, z; for (x of { x: async function() {} } = {});",
    "fileName": "__root__",
    "flags": 0,
    "start": 0,
    "end": 70
