...
let tests = [function (t: [number]) {
      t[1]; // error, out of bounds
      t[-1];
    } // error, out of bounds, // toString's to an invalid indexfunction (t: [
      number,
      string
    ]) {
      t[0.5]; // error: index out of bounds
      t[0.0000000000000000000001];
    } // error: index ("1e-22") out of bounds, // through a variablefunction (t: [
      number
    ]) {
      const x = 0.5;
      t[x];
    } // error, not an integer, // Return length from a function with mismatched return type.function () // error: tuple length 2 !~> string{
