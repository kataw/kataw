...
export function printKeyword(printer: any, keyword: any, parent: any, addSpace: boolean): any {
  if (keyword) {
    ...
    parts.push(addSpace ? concat([tokenToString(keyword.kind), ' ']) : tokenToString(keyword.kind));
...
function getLeadingCommentsWithoutDetachedComments(printer: Printer) {
  ...
  const leadingComments = collectLeadingComments(printer.source, pos, printer.consumedCommentRanges);
...
export function emitLeadingComments(printer: Printer, range: any) {
  const comments = getLeadingCommentsToEmit(printer, range);
...
function getLeadingCommentsToEmit(printer: Printer, pos: number) {
  if (printer.containerPos === -1 || pos !== printer.containerPos) {
    return hasDetachedComments(printer, pos)
      ...
      : collectLeadingComments(printer.source, pos, printer.consumedCommentRanges);
  }
  return '';
}
...
export function emitTrailingComments(printer: Printer, range: any) {
  ...
  return printComments(printer, trailingComments, /*leadingSeparator*/ true, /*trailingSeparator*/ false);
}
...
export function emitBodyWithDetachedComments(
  printer: Printer,
  detachedRange: any,
  lineMap: any,
  parentNode: any,
