...
export function printSource(root: RootNode, options?: PrinterOptions) {
	...
	if (options != null) {
		...
		if (options.computedPropertySpacing)
			flags |= PrinterFlags.ComputedPropertySpacing;
		if (options.allowArrowParens) flags |= PrinterFlags.ArrowParens;
		if (options.enforceLineBreaksBetweenArray)
			flags |= PrinterFlags.EnforceLineBreaksBetweenArray;
...
export function printStatementWorker(
	...
	if (map) {
		return map(printer, node, lineMap, parentNode);
	}
...
function printMemberAccessExpression(
	...
	const separator =
		printer.flags & PrinterFlags.ComputedPropertySpacing
			? printer.space
			: softline;
	return node.member === SyntaxKind.NumericLiteral ||
		node.member === SyntaxKind.BigIntLiteral
		? concat([
				printStatement(printer, node.member, lineMap, node),
				printPunctuator('[', printer, node.member.end, node),
				indent(
...
export function printRootNode(
	...
	return emitBodyWithDetachedComments(
		printer,
		node,
		lineMap,
		parentNode,
...
export function printRootNode1(
	printer: Printer,
	node: RootNode,
	lineMap: number[],
	parentNode: SyntaxNode,
	...
	if (directives && directives.length > 0) {
		...
		tokens.push(
			printer.flags & PrinterFlags.DisallowSemicolon && statements.length === 0
				? ''
				: ';',
			hardline,
		);
...
export function printElementList(
	...
	let lineBreaksBetweenArray =
		printer.flags & PrinterFlags.EnforceLineBreaksBetweenArray
			? hardline
			: softline;
	...
	return group(
		concat([
			...
			indent(
				concat([
					printer.flags & PrinterFlags.ArrayBracketSpacing
						? line
						: printer.flags &
						  (PrinterFlags.ArrayBracketNewline |
								PrinterFlags.EnforceLineBreaksBetweenArray)
						? hardline
						: softline,
...
function printTypeAnnotation(
	...
	return node.type.kind !== SyntaxKind.UnionType &&
		node.type.kind !== SyntaxKind.IntersectionType
		? concat([
...
function printObjectTypeInternalSlot(
	...
	return concat([
		...
		']',
		printPunctuator(']', printer, node.name.end, node),
		printKeyword(printer, node.optionalToken, node, /* addSpace */ true),
		node.type.kind === SyntaxKind.FunctionType
			? ''
...
function printIntersectionType(
	...
	return group(
		concat([
			parentNode.kind === SyntaxKind.TypeAnnotation &&
			parentNode.bitwiseAndToken
				? concat(['&', printer.space])
				: '',
			concat(result),
		]),
...
function printQualifiedType(
	...
	return concat([
		...
		printPunctuator('.', printer, node.qualification.end, node),
		printStatement(printer, node.name, lineMap, node),
	]);
}
...
function printTupleType(
	...
	return group(
		concat([
			...
			ifBreak(
				node.trailingComma && previousSibling.kind !== SyntaxKind.RestType
					? ','
					: '',
			),
			softline,
...
function printBindingProperty(
	...
	return concat([
		printKeyword(printer, node.ellipsisToken, node, /* addSpace */ false),
		printer.flags & PrinterFlags.QuoteProps &&
		node.key.kind === SyntaxKind.Identifier
			...
			: printer.flags & PrinterFlags.CoerceQuoteProps &&
...
function printFunctionStatementList1(
	printer: Printer,
	node: any,
	lineMap: number[],
	parentNode: SyntaxNode,
...
function printClassDeclarationOrExpression(
	...
	return concat([
		node.decorators
			? concat([
					parentNode.kind === SyntaxKind.ExportDeclaration ||
					parentNode.kind === SyntaxKind.ExportDeclaration
						? line
						: '',
...
function printObjectLiteral(
	...
	return printBindingPropertyListOrPropertyDefinitionList(
		printer,
		node.propertyList,
		lineMap,
		parentNode,
...
function printObjectBindingPattern(
	...
	return printBindingPropertyListOrPropertyDefinitionList1(
		printer,
		node.propertyList,
		lineMap,
		parentNode,
...
function printVariableStatement(
	...
	let printed = printVariableDeclarationList(
		printer,
	...
	const hasValue = node.declarationList.declarations.some(
		(decl: any) => decl.init,
	);
	let firstVariable;
	if (printed.length === 1) {
		...
		concat([
			...
			indent(
				concat(
					printed
						.slice(1)
						.map((p: any) => concat([',', hasValue ? hardline : line, p])),
				),
			),
			toggleSemicolon(printer),
		]),
	...
	return group(
		concat([
			...
			printPunctuator(
				'[',
				printer,
				node.chain ? node.chain.end : node.start,
				node,
			),
			separator,
			group(
...
function printNewTarget(printer: Printer, node: any): any {
	return concat([
		...
		'.',
		printKeywordNoSpace(printer, node.targetIdentifier),
	]);
}
	...
	if (node.clauses.length === 0) {
		return concat([
			...
			printPunctuator(
				'}',
				printer,
				node.clauses.length ? lastOrUndefined(node.clauses).end : node.end,
				node,
			),
		]);
	}
...
function printForOfStatement(
	...
	return group(
		concat([
			...
			printer.flags & PrinterFlags.NoWhiteSpace &&
			node.expression.transformFlags & TransformFlags.ArrayOrObjectLiteral
				? ''
				: ' ',
			printStatement(printer, node.expression, lineMap, node),
			printPunctuator(')', printer, node.expression.end, node),
		...
		if (node.kind === SyntaxKind.ForBinding) {
			...
			return group(
				concat([
					...
					firstVariable ? concat([' ', firstVariable]) : '',
					indent(
						concat(printed.slice(1).map((p: any) => concat([',', line, p]))),
					),
				]),
				{ shouldBreak: false },
			);
