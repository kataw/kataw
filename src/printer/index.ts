...
export function printStatementWorker(printer: Printer, node: SyntaxNode, lineMap: number[], parentNode: SyntaxNode) {
  if (node.flags & NodeFlags.IgnoreNextNode) return printer.source.slice(node.start, node.end);
...
export function printRootNode(
  ...
  return emitBodyWithDetachedComments(printer, node, lineMap, parentNode, printRootNode1);
}
      ...
      indent(
        concat([
          printer.flags & PrinterFlags.ArrayBracketSpacing
            ? line
            : printer.flags & (PrinterFlags.ArrayBracketNewline | PrinterFlags.EnforceLineBreaksBetweenArray)
            ? hardline
            : softline,
...
function printIntersectionType(printer: Printer, node: any, lineMap: number[], parentNode: any): any {
  ...
  return group(
    concat([
      parentNode.kind === SyntaxKind.TypeAnnotation && parentNode.bitwiseAndToken ? concat(['&', printer.space]) : '',
      concat(result)
    ]),
    {}
  );
...
function printUnionType(printer: Printer, node: UnionType, lineMap: number[], parentNode: any): any {
  ...
  return group(
    concat([
      parentNode.kind === SyntaxKind.TypeAnnotation && parentNode.bitwiseOrToken ? concat(['|', printer.space]) : '',
      concat(result)
    ]),
    {}
  );
...
function printFunctionStatementList1(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  const { directives, statements } = node;
...
function printFunctionStatementList(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  ...
  return concat([
    '{',
    indent(
      concat([hardline, emitBodyWithDetachedComments(printer, node, lineMap, parentNode, printFunctionStatementList1)])
    ),
    hardline,
    emitLeadingComments(printer, node.end),
    '}'
...
function printNewExpression(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return concat([
    ...
    printStatement(printer, node.expression, lineMap, node),
    node.argumentList ? printArgumentsList(printer, node.argumentList, lineMap, node) : ''
  ]);
}
...
function printImportDeclaration(printer: Printer, node: any, lineMap: number[]): any {
  return concat([
...
function printNamedImports(printer: Printer, node: any, lineMap: number[]): any {
...
function printCaseBlock(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  if (node.clauses.length === 0) {
    return concat([
      ...
      printTrailingCommentsOfPosition(printer, node.start),
      emitLeadingComments(printer, node.end),
      printPunctuator('}', printer, node.clauses.length ? lastOrUndefined(node.clauses).end : node.end, node)
    ]);
  }
