...
export function aladdin(diagnosticSource: DiagnosticSource, kind: DiagnosticKind,  message: string,  start: number, end: number, source: any) {
  ...
  process.stdout.write(`${lnSpacing}${ColorCodes.Yellow + '-->' + ColorCodes.Reset } in ${ColorCodes.Cyan +'foo.js' + ColorCodes.Reset}\n\n`);
  process.stdout.write(`${highlightColor + lineNumber + ColorCodes.Reset + ColorCodes.White + ' |' + ColorCodes.Reset} ${line}\n`);
