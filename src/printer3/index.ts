    ...
    : group(
        concat([
          ...
          operatorToken.flags & NodeFlags.IgnoreNextNode
            ...
            ? group(indent(concat([line, printStatement(printer, right, lineMap, node)])), {})
...
export function printRootNode(
  ...
  if (directives) {
    for (let i = 0; i < directives.length; i++) {
      tokens.push(concat([hardline, printStatement(printer, directives[i], lineMap, parentNode), ';']));
    }
...
export function printElementList(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  ...
  return group(
    concat(['[', indent(concat([lineBreak, concat([concat(elements), trailingComma])])), lineBreak, ']']),
    {}
    // shouldBreak
  );
}
...
export function printerBindingElementList(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  if (node.elements.length === 0) {
    return '[]';
  }
...
function printFunctionTypeParameter(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return node.flags & NodeFlags.IgnoreNextNode
    ...
    : concat([
        ...
        printKeyword(printer, node.optionalToken, node, /* addSpace */ true),
        node.types
          ? concat([node.name ? concat([':', ' ']) : '', printStatement(printer, node.types, lineMap, node)])
          : ''
      ]);
}
...
function printOpaqueType(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return node.flags & NodeFlags.IgnoreNextNode
    ? printer.source.slice(node.start, node.end)
    : node.declareToken
    ? concat([
    ...
    : concat([
        ...
        printStatement(printer, node.name, lineMap, node),
        node.typeParameters ? printStatement(printer, node.typeParameters, lineMap, node) : '',
        node.superType ? concat([':', ' ', printStatement(printer, node.superType, lineMap, node)]) : '',
        ' ',
        '=',
        ' ',
...
function printOptionalIndexedAccess(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return concat([
    printStatement(printer, node.objectType, lineMap, node),
...
function printTupleType(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  ...
  return group(concat(['[', indent(concat([softline, concat(elements)])), softline, ']']), {});
}
...
function printBindingProperty(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return concat([
    ...
    ' ',
    printStatement(printer, node.value, lineMap, node),
    node.initializer ? concat([' ', '=', ' ', printStatement(printer, node.initializer, lineMap, node)]) : ''
  ]);
}
...
function printFunctionDeclarationOrExpression(
  ...
  return node.flags & NodeFlags.IgnoreNextNode
    ...
    : concat([
        ...
        ' ',
        node.contents ? printFunctionBody(printer, node.contents, lineMap, node) : ''
      ]);
}
...
function printClassTail(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return concat([
    node.classHeritage ? concat([' ', printStatement(printer, node.classHeritage, lineMap, node)]) : '',
...
function printFieldDefinition(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return concat([
    ...
    node.type
      ...
      : '',
    node.initializer ? printInitializer(printer, node.initializer, lineMap, node) : '',
    ';'
  ]);
}
...
function printObjectType(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  ...
  return group(concat(['{', indent(concat([line, concat(elements)])), line, '}']), {});
}
  ...
  return node.flags & NodeFlags.IgnoreNextNode
    ...
    : group(
        concat([
          ...
          'var',
          ' ',
          indent(printVariableDeclarationList(printer, node.declarationList, lineMap, node)),
          inForStatement ? '' : ';'
        ]),
        {}
      );
...
function printLexicalDeclaration(
  ...
  return node.flags & NodeFlags.IgnoreNextNode
    ? printer.source.slice(node.start, node.end)
    : group(concat([tokenToString(node.lexicalKeyword), ' ', indent(printed), inForStatement ? '' : ';']), {});
}
...
function printExportSpecifier(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return concat([
    ...
    node.binding ? concat([' ', printStatement(printer, node.binding, lineMap, node)]) : ''
  ]);
}

function printExportDefault(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return node.flags & NodeFlags.IgnoreNextNode
    ? printer.source.slice(node.start, node.end)
    : concat([
...
function printWhileStatement(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return node.flags & NodeFlags.IgnoreNextNode
    ? printer.source.slice(node.start, node.end)
    : group(
        concat([
...
function printDebuggerStatement(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return concat([printKeyword(printer, node.debuggerKeyword, node, /* addSpace */ false), ';']);
}
...
function printContinueStatement(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return node.flags & NodeFlags.IgnoreNextNode
    ? printer.source.slice(node.start, node.end)
    : concat([
        printKeyword(printer, node.continueKeyword, node, /* addSpace */ true),
        node.label ? concat([' ', printStatement(printer, node.label, lineMap, node)]) : '',
        ';'
      ]);
}

function printBreakStatement(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return node.flags & NodeFlags.IgnoreNextNode
    ? printer.source.slice(node.start, node.end)
    : concat([
...
function printWithStatement(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return node.flags & NodeFlags.IgnoreNextNode
    ? printer.source.slice(node.start, node.end)
    : group(
        concat([
...
function printReturnStatement(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return concat([
...
function printThrowStatement(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return concat([
...
function printIfStatement(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  const parts: any[] = [
    group(
      concat([
...
function printClassHeritage(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return concat([
...
function printParenthesizedExpression(printer: Printer, node: any, lineMap: number[], parentNode: SyntaxNode): any {
  return group(
    concat(['(', indent(concat([softline, printStatement(printer, node.expression, lineMap, node)])), softline, ')']),
    {}
  );
