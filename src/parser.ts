...
export function parse(source: string, filename: string, isModule: boolean): RootNode {
  ...
  return createRootNode(directives, statements, isModule, source, filename, parser.diagnostics);
}
...
function parseStatementListItem(parser: ParserState, context: Context): StatementNode {
  switch (parser.token) {
    ...
    case SyntaxKind.AsyncKeyword:
      return parseFunctionDeclaration(parser, context, /* disallowGen*/ false);
    case SyntaxKind.Decorator:
    case SyntaxKind.ClassKeyword:
      return parseClassDeclaration(parser, context, /* declareToken */ null);
    ...
    case SyntaxKind.TypeKeyword:
      return parseTypeAsIdentifierOrTypeAlias(parser, context, /* opaqueToken */ null, /* declareToken */ null);
    case SyntaxKind.OpaqueKeyword:
      return parseOpaqueType(parser, context, /* declareToken */ null);
    ...
    case SyntaxKind.ExportKeyword:
      parser.diagnostics.push(
        createDiagnosticError(
          DiagnosticSource.Parser,
          DiagnosticCode.The_export_keyword_can_only_be_used_with_the_module_goal,
          parser.curPos,
          parser.pos
        )
      );
...
function parseStatement(parser: ParserState, context: Context, allowFunction: boolean): StatementNode {
  switch (parser.token) {
    case SyntaxKind.VarKeyword:
      return parseVariableStatement(parser, context, /* declareToken */ null);
    ...
    case SyntaxKind.ClassKeyword:
      parser.diagnostics.push(
        createDiagnosticError(
          DiagnosticSource.Parser,
          DiagnosticCode.Class_declarations_are_not_allowed_in_an_arbitrary_statement_position,
          parser.curPos,
          parser.pos
        )
      );
...
function parseConsequentOrAlternative(parser: ParserState, context: Context): StatementNode {
  ...
  return context & (Context.OptionsDisableWebCompat | Context.Strict) || parser.token !== SyntaxKind.FunctionKeyword
    ...
    : parseFunctionDeclaration(parser, context, /* disallowGen */ true);
}
...
function parseWithStatement(parser: ParserState, context: Context): WithStatement {
  ...
  if (context & Context.Strict) {
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode._with_statements_are_not_allowed_in_strict_mode,
        pos,
        parser.pos
      )
    );
...
function parseThrowStatement(parser: ParserState, context: Context): ThrowStatement {
  ...
  if (parser.nodeFlags & NodeFlags.NewLine) {
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode.Linebreak_ermitted_after_throw,
        parser.curPos,
        parser.pos
      )
    );
...
function parseForStatement(parser: ParserState, context: Context): ForStatement | ForInStatement | ForOfStatement {
        ...
        if ((parser.token as SyntaxKind) === SyntaxKind.InKeyword) {
          if (context & Context.Strict) {
            parser.diagnostics.push(
              createDiagnosticError(
                DiagnosticSource.Parser,
                DiagnosticCode.Identifier_expected_let_is_a_reserved_word_in_strict_mode,
                parser.curPos,
                parser.pos
              )
            );
  ...
  if (
    ...
    if (!parser.assignable) {
      parser.diagnostics.push(
        createDiagnosticError(
          DiagnosticSource.Parser,
          DiagnosticCode.Invalid_left_hand_side_in_for_of_loop,
          parser.curPos,
          parser.pos
        )
      );
  ...
  if (consumeOpt(parser, context | Context.AllowRegExp, SyntaxKind.InKeyword)) {
    if (!parser.assignable) {
      parser.diagnostics.push(
        createDiagnosticError(
          DiagnosticSource.Parser,
          DiagnosticCode.Invalid_left_hand_side_in_for_in_loop,
          parser.curPos,
          parser.pos
        )
      );
  ...
  if (parser.destructible & DestructibleKind.MustDestruct) {
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode._can_only_be_used_in_an_object_literal_property_inside_a_destructuring,
        parser.curPos,
        parser.pos
      )
    );
...
function parseIdentifier(
  ...
  parser.diagnostics.push(
    createDiagnosticError(
      DiagnosticSource.Parser,
      diagnosticMessage ? diagnosticMessage : DiagnosticCode.Identifier_expected,
      parser.curPos,
      parser.pos
    )
  );
...
function parseOptionalChain(parser: ParserState, context: Context): any {
  ...
  if (consumeOpt(parser, context | Context.AllowRegExp, SyntaxKind.LeftBracket)) {
    ...
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode.Tagged_template_expressions_are_not_permitted_in_an_optional_chain,
        parser.curPos,
        parser.pos
      )
    );
    ...
    if (consumeOpt(parser, context | Context.AllowRegExp, SyntaxKind.Period)) {
      ...
      if ((chain as any).expression.kind === SyntaxKind.PrivateIdentifier) {
        parser.diagnostics.push(
          createDiagnosticError(
            DiagnosticSource.Parser,
            DiagnosticCode.An_optional_chain_cannot_contain_private_identifiers,
            parser.curPos,
            parser.pos
          )
        );
...
function parsePropertyOrPrivatePropertyName(parser: ParserState, context: Context): Identifier | PrivateIdentifier {
  ...
  parser.diagnostics.push(
    createDiagnosticError(
      DiagnosticSource.Parser,
      DiagnosticCode.Dot_property_must_be_an_identifier,
      parser.curPos,
      parser.pos
    )
  );
...
function parseMemberAccessExpression(
  ...
  if (parser.token === SyntaxKind.RightBracket) {
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode.An_element_access_expression_should_take_an_argument,
        parser.curPos,
        parser.pos
      )
    );
...
function parseArrowFunction(
  ...
  if (parser.nodeFlags & NodeFlags.NewLine) {
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode.Line_terminator_not_permitted_before_arrow,
        parser.curPos,
        parser.pos
      )
    );
      ...
      switch (parser.token as SyntaxKind) {
        ...
        case SyntaxKind.Multiply:
          parser.diagnostics.push(
            createDiagnosticError(
              DiagnosticSource.Parser,
              DiagnosticCode.An_arrow_function_can_not_be_part_of_an_operator_to_the_right,
              parser.curPos,
              parser.pos
            )
          );
        ...
        switch (parser.token as SyntaxKind) {
          ...
          case SyntaxKind.LeftBracket:
            parser.diagnostics.push(
              createDiagnosticError(
                DiagnosticSource.Parser,
                DiagnosticCode.Block_body_arrows_can_not_be_immediately_invoked_without_a_group,
                parser.curPos,
                parser.pos
              )
            );
...
function parseIdentifierReference(parser: ParserState, context: Context): Identifier {
  ...
  if (token & (SyntaxKind.IsFutureReserved | SyntaxKind.IsIdentifier)) {
    ...
    if (context & (Context.Module | Context.InAwaitContext) && token === SyntaxKind.AwaitKeyword) {
      parser.diagnostics.push(
        createDiagnosticError(
          DiagnosticSource.Parser,
          DiagnosticCode._let_is_a_restricted_production_at_the_start_of_a_statement,
          parser.curPos,
          parser.pos
        )
      );
  ...
  parser.diagnostics.push(
    createDiagnosticError(DiagnosticSource.Parser, DiagnosticCode.Identifier_expected, parser.curPos, parser.pos)
  );
...
function parsePropertyDefinition(
  ...
  if (parser.token & SyntaxKind.IsLessThanOrLeftParen) {
    return parseMethodDefinition(parser, context, key, nodeFlags);
  }
      ...
      if (parser.token !== SyntaxKind.RightBrace && parser.token !== SyntaxKind.Comma) {
        if (parser.destructible & DestructibleKind.MustDestruct) {
          parser.diagnostics.push(
            createDiagnosticError(
              DiagnosticSource.Parser,
              DiagnosticCode.Invalid_optional_chain_from_new_expression,
              parser.curPos,
              parser.pos
            )
          );
...
function parseMethodDefinition(
  ...
  const content = parseFunctionBody(
    parser,
...
function parsentheizedExpression(parser: ParserState, context: Context): ParenthesizedExpression | any {
  ...
  if (consumeOpt(parser, context, SyntaxKind.RightParen)) {
    ...
    switch (parser.token) {
      ...
      default:
        parser.diagnostics.push(
          createDiagnosticError(DiagnosticSource.Parser, DiagnosticCode.Expression_expected, parser.curPos, parser.pos)
        );
      ...
      if (parser.token === SyntaxKind.Arrow || returnType) {
        if (destructible & (DestructibleKind.Assignable | DestructibleKind.NotDestructible)) {
          parser.diagnostics.push(
            createDiagnosticError(
              DiagnosticSource.Parser,
              DiagnosticCode.The_left_hand_side_of_the_arrow_is_not_destructible,
              parser.curPos,
              parser.pos
            )
          );
...
function parseArrayBindingElement(parser: ParserState, context: Context): OmittedExpression | ArrayBindingElement {
  ...
  if (ellipsisToken && (parser.token as SyntaxKind) === SyntaxKind.Assign) {
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode.A_rest_element_cannot_have_an_initializer,
        parser.curPos,
        parser.pos
      )
    );
...
function parseBindingProperty(parser: ParserState, context: Context): BindingProperty | SingleNameBinding {
  ...
  if (ellipsisToken) {
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode.The_binding_pattern_is_not_destructible,
        parser.curPos,
        parser.pos
      )
    );
...
function parseFunctionDeclaration(
  ...
  if (disallowGen && generatorToken) {
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode.Generators_can_only_be_declared_at_the_top_level_or_inside_a_block,
        parser.curPos,
        parser.pos
      )
    );
...
function parseImportDeclaration(
  ...
  if (isScript) {
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode.The_import_keyword_can_only_be_used_with_the_module_goal,
        parser.curPos,
        parser.pos
      )
    );
...
function parseExportDeclaration(
  ...
  switch (parser.token) {
    ...
    case SyntaxKind.Decorator:
    case SyntaxKind.ClassKeyword:
      declaration = parseClassDeclaration(parser, context, /* declareToken */ null);
      break;
    case SyntaxKind.FunctionKeyword:
      declaration = parseFunctionDeclaration(parser, context, /* disallowGen*/ true);
      break;
    case SyntaxKind.VarKeyword:
      declaration = parseVariableStatement(parser, context, /* declareToken */ null);
      break;
    case SyntaxKind.AsyncKeyword:
    default:
      parser.diagnostics.push(
...
function parseExportDefault(
  ...
  switch (parser.token) {
    case SyntaxKind.FunctionKeyword:
      declaration = parseFunctionDeclaration(parser, context, /* disallowGen*/ false);
      break;
    case SyntaxKind.Decorator:
    case SyntaxKind.ClassKeyword:
      declaration = parseClassDeclaration(parser, context, /* declareToken */ null);
...
function parseFunctionTypeOrParen(parser: ParserState, context: Context): any {
  ...
  if ((parser.token as SyntaxKind) !== SyntaxKind.RightParen && (parser.token as SyntaxKind) !== SyntaxKind.Ellipsis) {
    if ((parser.token as SyntaxKind) === SyntaxKind.Identifier) {
      const pos = parser.curPos;
      const type = parseIdentifier(parser, context);
      if (
        ...
        consume(parser, context, SyntaxKind.Colon);
        const returnType = parseType(parser, context);
        const params = parseFunctionTypeParameterssRest(
          parser,
          context,
        ...
        return createFunctionType(params, parseType(parser, context), /* typeParameters */ null, pos, parser.curPos);
      }
      consume(parser, context, SyntaxKind.RightParen);
      return type;
    }
...
function parseVariableStatement(
  ...
  return createVariableStatement(
    ...
    varToken,
    declarationList,
    declareToken ? NodeFlags.IsStatement | NodeFlags.Declared : NodeFlags.IsStatement,
    pos,
    parser.curPos
  );
}
  ...
  if (consumeOpt(parser, context | Context.AllowRegExp, SyntaxKind.Assign)) {
    ...
    if (context & Context.InAwaitContext && parser.token === SyntaxKind.AwaitKeyword) {
      parser.diagnostics.push(
        createDiagnosticError(
          DiagnosticSource.Parser,
          DiagnosticCode._await_expressions_cannot_be_used_in_a_parameter_initializer,
          parser.curPos,
          parser.pos
        )
      );
...
function parseDeclareAsIdentifierOrDeclareStatement(
  ...
  if (parser.token & SyntaxKind.IsStatementStart) {
    const declareToken = createToken(SyntaxKind.DeclareKeyword, expr.start, expr.end);
    if (parser.token === SyntaxKind.VarKeyword) {
      return parseVariableStatement(parser, context, declareToken);
    }
    if (parser.token === SyntaxKind.TypeKeyword) {
      return parseTypeAsIdentifierOrTypeAlias(parser, context, /* opaqueToken */ null, /* declareToken */ declareToken);
    }
    if (parser.token === SyntaxKind.OpaqueKeyword) {
      return parseOpaqueType(parser, context, /* declareToken */ declareToken);
    }

    if (parser.token === SyntaxKind.ClassKeyword) {
      return parseClassDeclaration(parser, context, /* declareToken */ declareToken);
    }
  }
  parser.assignable = true;
  if (parser.token === SyntaxKind.Colon) {
...
function parseLexicalDeclaration(parser: ParserState, context: Context, isConst: boolean): LexicalDeclaration {
  const pos = parser.curPos;
  const lexicalToken = consumeToken(parser, context, isConst ? SyntaxKind.ContinueKeyword : SyntaxKind.LetKeyword);
...
function parseLexicalBinding(
  ...
  if (isConst && parser.token === SyntaxKind.LetKeyword) {
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode.A_lexical_declaration_can_t_define_a_let_binding,
        parser.curPos,
        parser.pos
      )
    );
  ...
  while ((parser.token as SyntaxKind) !== SyntaxKind.RightBrace) {
    ...
    if (allowStatic && consumeOpt(parser, context, SyntaxKind.StaticKeyword)) {
      if (parser.token & SyntaxKind.IsLessThanOrLeftParen) {
        ...
        indexers.push(parseObjectTypeIndexer(parser, context, /* isStatic */ staticToken, innerPos));
      }
    } else {
      if (consumeOpt(parser, context, SyntaxKind.LeftBracket)) {
        if (parser.token === SyntaxKind.LeftBracket) {
          internalSlots.push(parseObjectTypeInternalSlot(parser, context, staticToken, innerPos));
        } else {
        ...
        properties.push(parseObjectTypeProperty(parser, context, staticToken, innerPos));
      }
    }
  ...
  if (parser.token & SyntaxKind.IsIdentifier) {
    ...
    if (token === SyntaxKind.GetKeyword) {
      return createObjectTypeProperty(
        /* key */ parseIdentifier(parser, context),
        /* value */ parseFunctionType1(parser, context),
        /* optionalToken */ null,
        /* staticToken */ null,
        pos,
        parser.curPos
...
function parseObjectTypeInternalSlot(
  ...
  if (parser.token & SyntaxKind.IsLessThanOrLeftParen) {
    ...
    return createObjectTypeInternalSlot(name, /* optionalToken */ null, staticToken, value, pos, parser.curPos);
  }
  const optionalToken = consumeOptToken(parser, context | Context.AllowRegExp, SyntaxKind.QuestionMark);
  consume(parser, context, SyntaxKind.Colon);
...
function parseObjectTypeIndexer(
  ...
  const value = parseType(parser, context);
  parseSemicolon(parser, context);
  return createObjectTypeIndexer(name, key, value, staticToken, pos, parser.curPos);
}
...
export function parseImportCall(
  ...
  if (parser.token === SyntaxKind.RightParen) {
    parser.diagnostics.push(
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode.Import_Call_should_take_at_least_one_argument,
        parser.curPos,
        parser.pos
      )
    );
...
function parseClassDeclaration(
  ...
  const classElementList = consume(parser, context, SyntaxKind.LeftBrace)
    ? parseClassElementList(parser, inheritedContext, context, /* isDecl */ true)
    : // Empty list
  ...
  if (token & (SyntaxKind.IsIdentifier | SyntaxKind.IsFutureReserved)) {
    let key: any =
      token === SyntaxKind.PrivateIdentifier
        ? parsePrivateIdentifier(parser, context)
        : parseIdentifier(parser, context);
    if ((parser.token & SyntaxKind.IsLessThanOrLeftParen) === 0) {
      switch (token) {
    ...
    if (nodeFlags & (NodeFlags.Async | NodeFlags.Getter | NodeFlags.Setter)) {
      if (
        ...
        parser.diagnostics.push(
          createDiagnosticError(
            DiagnosticSource.Parser,
            DiagnosticCode.Method_definition_expected,
            parser.curPos,
            parser.pos
          )
        );
    ...
    if (parser.tokenValue === 'constructor') {
      if (!staticToken && parser.token & SyntaxKind.IsLessThanOrLeftParen) {
        if (nodeFlags & (NodeFlags.Async | NodeFlags.Getter | NodeFlags.Setter)) {
          parser.diagnostics.push(
            createDiagnosticError(
              DiagnosticSource.Parser,
        ...
        if ((context & Context.SuperCall) !== Context.SuperCall) {
          if (parser.nodeFlags & NodeFlags.HasConstructor) {
            parser.diagnostics.push(
              createDiagnosticError(
                DiagnosticSource.Parser,
                DiagnosticCode.Duplicate_constructor_method_in_class,
                parser.curPos,
                parser.pos
              )
            );
      ...
      createDiagnosticError(
        DiagnosticSource.Parser,
        DiagnosticCode.Classes_may_not_have_a_static_property_named_prototype,
        parser.curPos,
        parser.pos
      );
    }
...
export function parseCoverCallExpressionAndAsyncArrowHead(
  ...
  if (consumeOpt(parser, context, SyntaxKind.RightParen)) {
    ...
    if (
      ...
      return parseArrowFunction(parser, context, typeParameters, returnType, [], asyncToken, start);
    }
...
function parseOpaqueType(parser: ParserState, context: Context, declareToken: SyntaxToken<TokenSyntaxKind> | null) {
  const pos = parser.curPos;
  let expr = parseIdentifier(parser, context);
  if (parser.token === SyntaxKind.TypeKeyword) {
